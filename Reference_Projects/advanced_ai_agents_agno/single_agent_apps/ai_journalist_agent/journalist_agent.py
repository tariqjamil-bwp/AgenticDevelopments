...
openai_api_key = os.getenv("GEMINI_API_KEY") #st.text_input("Enter OpenAI API Key to access GPT-4o", type="password")

# Get SerpAPI key from the user
serp_api_key = os.getenv("SERPER_API_KEY") #st.text_input("Enter Serp API Key for Search functionality", type="password")

if openai_api_key and serp_api_key:
    searcher = Agent(
        name="Searcher",
        role="Searches for top URLs based on a topic",
        model= Gemini(api_key=openai_api_key),
        description=dedent(
            """\
    ...
    writer = Agent(
        ...
        model=Gemini(id="gemini-2.0-flash", api_key=openai_api_key),
        description=dedent(
            """\
