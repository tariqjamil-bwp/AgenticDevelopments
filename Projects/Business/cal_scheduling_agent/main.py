...
try:
    calcom_tools = CalComTools(
        ...
        event_type_id=EVENT_TYPE_ID  # Make sure to set this
    )
...
agent = Agent(
    ...
    model=LiteLLM(
        ...
        api_key=os.getenv("GOOGLE_API_KEY") # Explicitly pass the API key
    ),
    tools=[calcom_tools],
...
def book_example_call():
    ...
    Please remove the booking with these details:
    - Start Time: 2025-09-09T12:30:00+05:30
    - Name: Gradeant
    - Email: tariqjamil.bwp@gmail.com
    - Booking UID: kaDugznDB27dbRUbAtfBLa
    
    After removing the booking, please verify the booking does not exist.
    """)
