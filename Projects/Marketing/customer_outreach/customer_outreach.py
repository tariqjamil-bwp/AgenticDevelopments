# -*- coding: utf-8 -*-
"""
Google ADK Customer Outreach Multi-Agent System - COMPLETE VERSION WITH OPENTELEMETRY FIX
All original functionality preserved with proper error suppression
"""
...
# =============================================================================
# COMPLETE OPENTELEMETRY ERROR SUPPRESSION
# =============================================================================

# Suppress OpenTelemetry warnings and errors
warnings.filterwarnings("ignore", category=UserWarning, module="opentelemetry")
warnings.filterwarnings("ignore", message=".*Token.*different Context.*")

# Configure logging to suppress OpenTelemetry errors
logging.getLogger("opentelemetry").setLevel(logging.CRITICAL)
logging.getLogger("opentelemetry.context").setLevel(logging.CRITICAL)
logging.getLogger("opentelemetry.trace").setLevel(logging.CRITICAL)

# Monkey patch the problematic detach method
import opentelemetry.context as otel_context
original_detach = otel_context._RUNTIME_CONTEXT.detach

def safe_detach(token):
    """Safe detach that suppresses context errors"""
    try:
        return original_detach(token)
    except (ValueError, RuntimeError) as e:
        # Silently ignore context detachment errors
        pass

otel_context._RUNTIME_CONTEXT.detach = safe_detach

# Also patch the contextvars context implementation
try:
    from opentelemetry.context.contextvars_context import ContextVarsRuntimeContext
    original_contextvars_detach = ContextVarsRuntimeContext.detach
    
    def safe_contextvars_detach(self, token):
        try:
            return original_contextvars_detach(self, token)
        except (ValueError, RuntimeError):
            pass
    
    ContextVarsRuntimeContext.detach = safe_contextvars_detach
except ImportError:
    pass
...
if not os.path.exists(instructions_dir):
    ...
    print("Reading from Instructions' files.")
    with open(os.path.join(instructions_dir, "company_research_guidelines.txt"), 'r', encoding='utf-8') as f:
        company_research_guidelines = f.read()
    with open(os.path.join(instructions_dir, "outreach_best_practices.txt"), 'r', encoding='utf-8') as f:
        outreach_best_practices = f.read()
    with open(os.path.join(instructions_dir, "ideal_customer_profile.txt"), 'r', encoding='utf-8') as f:
        ideal_customer_profile = f.read()
...
async def analyze_sentiment_async(text: str) -> str:
    """Async wrapper for SentimentAnalysisTool with error handling"""
    try:
        # Truncate text to avoid memory issues
        text_sample = text[:200] if len(text) > 200 else text
        print(f"📊 Analyzing sentiment for text: {text_sample}...")
        
        # Run in executor to avoid blocking
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, sentiment_analysis_tool._run, text)
        return str(result)
    except Exception as e:
...
# -----------------------------
# Initialize CrewAI tools with CrewAITool wrapper
directory_tool = CrewaiTool(DirectoryReadTool(directory=instructions_dir), name="directory_tool", description="Reads the contents of a directory.")
file_tool = CrewaiTool(FileReadTool(), name="file_tool", description="Reads the contents of a file.")
search_tool = CrewaiTool(EXASearchTool(), name="search_tool", description="Searches the web for information.")
...
lead_research_agent = LlmAgent(
    ...
    1. **search_tool**: Web search for recent company information, news, and developments
    2. **directory_tool**: Access research guidelines and best practices from instructions directory
    3. **file_tool**: Read specific files containing company data, guidelines, or customer profiles
    4. **sentiment_tool**: Analyze sentiment of company communications and content
...
campaign_strategist_agent = LlmAgent(
    ...
    1. **search_tool**: Additional research for campaign personalization
    2. **directory_tool**: Access outreach best practices and templates
    3. **file_tool**: Read specific campaign guidelines and customer success stories
    4. **sentiment_tool**: Ensure all campaign messaging maintains positive, engaging tone
...
# =============================================================================
@asynccontextmanager
async def safe_runner_context(runner: Runner, user_id: str, session_id: str, message: types.Content):
    """Context manager to safely handle async generator and context cleanup"""
    generator = None
    try:
        generator = runner.run_async(user_id=user_id, session_id=session_id, new_message=message)
        yield generator
    except Exception as e:
        print(f"⚠️ Error in runner context: {e}")
        raise
    finally:
        if generator:
            try:
                ...
                # Suppress any context cleanup errors
                pass
...
async def run_comprehensive_outreach(campaign_inputs: dict):
    """Run comprehensive outreach campaign with proper async handling."""
    ...
    try:
        ...
        async with safe_runner_context(runner, USER_ID, SESSION_ID, content) as event_generator:
            async for event in event_generator:
                ...
                if event.is_final_response():
                    ...
                    if agent_name == "lead_research_agent":
                        fname = f"lead_research_{company_safe_name}.md"
                        ...
                        fname = f"campaign_strategy_{company_safe_name}.md"
                        filename = os.path.join(OUTPUT_DIR, fname)
...
# =============================================================================
# BATCH PROCESSING WITH ENHANCED REPORTING (FIXED)
# =============================================================================
async def run_batch_outreach_campaigns(campaign_list: List[dict], batch_name: str = "batch_campaign"):
    """Run multiple campaigns with comprehensive reporting and proper error handling."""
    ...
    for i, campaign_inputs in enumerate(campaign_list, 1):
        ...
        try:
            ...
            # Brief pause between campaigns to avoid rate limiting
    ...
    batch_report_fname = f"{batch_name}_report.md"
    batch_report_filename = os.path.join(OUTPUT_DIR, batch_report_fname)
...
def validate_campaign_inputs(campaign_inputs: dict) -> bool:
    """Validate campaign input parameters"""
    required_fields = ['lead_name', 'industry', 'key_decision_maker', 'position', 'milestone']
    
    for field in required_fields:
        if field not in campaign_inputs or not campaign_inputs[field]:
...
async def health_check():
    """Check if all required services and APIs are accessible"""
...
async def cleanup_old_files(days_old: int = 7):
    """Clean up old campaign files"""
    import time
    import glob
    
    current_time = time.time()
    patterns = ["lead_research_*.md", "campaign_strategy_*.md", "*_report.md"]
    patterns = [os.path.join(OUTPUT_DIR, pattern) for pattern in patterns]
...
async def run_comprehensive_outreach_with_retry(campaign_inputs: dict, max_retries: int = 3):
    """Run comprehensive outreach with retry logic"""
    
    if not validate_campaign_inputs(campaign_inputs):
        return None
    
    for attempt in range(max_retries):
        try:
            print(f"🔄 Attempt {attempt + 1}/{max_retries} for {campaign_inputs['lead_name']}")
            result = await run_comprehensive_outreach(campaign_inputs)
            
            if result:
                print(f"✅ Successfully completed campaign for {campaign_inputs['lead_name']}")
                return result
            else:
                print(f"⚠️ No results returned on attempt {attempt + 1}")
            ...
            if attempt < max_retries - 1:
                ...
                print(f"💀 All {max_retries} attempts failed for {campaign_inputs['lead_name']}")
                return None
...
async def main():
    ...
    single_campaign = {
        "lead_name": "Naqshar",
        "industry": "Architect Designing", 
        "key_decision_maker": "Hamza",
        "position": "CTO",
        "milestone": "Architect Designing",
        "objective": "Architect Designing Campaign1"
    }
    ...
    try:
        ...
        if choice == "1":
            ...
            results = await run_comprehensive_outreach_with_retry(single_campaign)
            ...
            if results:
                ...
                print("❌ Campaign failed after all retry attempts")
            ...
            cleanup_choice = input("Clean up old campaign files? (y/n): ").strip().lower()
            if cleanup_choice == 'y':
                await cleanup_old_files()
            
            results = await run_batch_outreach_campaigns(batch_campaigns, "ai_companies_batch")
